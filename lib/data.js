...
async function fetchAll() {
  ...
  const [booksRes, epRes, revRes, preRes, statsRes] = await Promise.all([
    ...
    reviewClient.from("book_asset_stats").select("*"),
  ]);
  ...
  const statsByBook = {};
  if (statsRes.error && !/book_asset_stats/i.test(statsRes.error.message)) {
    console.error("asset stats fetch failed:", statsRes.error.message);
  }
  for (const s of statsRes.data || []) {
    (statsByBook[s.book_slug] ||= []).push(s);
  }

  const dbBooks = (booksRes.data || []).map((row) =>
    rowToBook(row, episodesByBook[row.slug] || [], reviewsByBook[row.slug] || [], statsByBook[row.slug] || [])
  );
...
export async function getBookBySlug(slug) {
  ...
  const [epRes, revRes, statsRes] = await Promise.all([
    ...
    reviewClient.from("book_asset_stats").select("*").eq("book_slug", slug),
  ]);

  if (statsRes.error && !/book_asset_stats/i.test(statsRes.error.message)) {
    console.error(`asset stats fetch failed for ${slug}:`, statsRes.error.message);
  }
  const book = rowToBook(row, epRes.data || [], revRes.data || [], statsRes.data || []);
  return {
    ...book,
    episodes: (book.episodes || []).filter((episode) => episode.published !== false),
  };
