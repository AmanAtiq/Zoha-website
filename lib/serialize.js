...
export function rowToAssetStats(row) {
  return {
...
function ratingFromReviews(reviews) {
  ...
  const counts = [5, 4, 3, 2, 1].map((star) => rated.filter((r) => Number(r.rating) === star).length);
  const total = rated.reduce((sum, r) => sum + Number(r.rating), 0);
  return {
    avg: Math.round((total / rated.length) * 10) / 10,
    count: rated.length,
    dist: counts.map((count) => Math.round((count / rated.length) * 100)),
  };
}

export function rowToBook(row, episodes = [], reviews = [], statsRows = []) {
  if (!row) return null;
  ...
  const allReviews = (reviews || []).map(rowToReview).filter(Boolean);
  const bookReviews = allReviews.filter((r) => !r.episodeSlug);
  const reviewsByEpisode = {};
  for (const review of allReviews) {
    if (!review.episodeSlug) continue;
    (reviewsByEpisode[review.episodeSlug] ||= []).push(review);
  }
  const manualRating = hasRating
    ? {
    ...
    : undefined;
  const bookPageReviews = row.type === "episodic" ? allReviews : bookReviews;
  const bookRating = ratingFromReviews(bookPageReviews) || manualRating;
  const statsByEpisode = {};
  for (const statsRow of statsRows || []) {
    const stats = rowToAssetStats(statsRow);
    statsByEpisode[stats.episodeSlug || ""] = stats;
  }
  const showReadDownloadStats = !!row.show_read_download_stats;
  ...
  return {
    ...
    prebookOnly: !!row.prebook_only,
    published: row.published ?? true,
    comingSoon: row.coming_soon || false,
    showReadDownloadStats,
    assetStats: showReadDownloadStats ? statsByEpisode[""] : undefined,
    episodes: (episodes || []).map((episodeRow) => {
      const episode = rowToEpisode(episodeRow);
      const episodeReviews = reviewsByEpisode[episode.slug] || [];
      return {
        ...episode,
        reviews: episodeReviews,
        rating: ratingFromReviews(episodeReviews),
        assetStats: showReadDownloadStats ? statsByEpisode[episode.slug] : undefined,
      };
    }),
    reviews: bookPageReviews,
    allReviews,
    rating: bookRating,
  };
}
...
export function bookToRow(book) {
  ...
  return {
    ...
    prebook_only: !!book.prebookOnly,
    published: book.published ?? true,
    coming_soon: !!book.comingSoon,
    show_read_download_stats: !!book.showReadDownloadStats,
  };
}
